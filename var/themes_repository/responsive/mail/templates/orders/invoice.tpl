                ...
                <td style="padding-top: 14px;">
                    ...
                    {if $tracking_number_exists }
                        <tr valign="top">
                            ...
                            <td style="font-size: 12px; font-family: Arial;">
                                {foreach from=$order_info.shipping item="shipping" name="f_shipp"}
                                    {if $shipments[$shipping.group_key].carrier}
                                        <p>{__("carrier")}: {$shipments[$shipping.group_key].carrier_info.name nofilter}{if $shipments[$shipping.group_key].tracking_number} ({__("tracking_number")}: {if $shipments[$shipping.group_key].carrier_info.tracking_url}<a target="_blank" href="{$shipments[$shipping.group_key].carrier_info.tracking_url nofilter}">{/if}{$shipments[$shipping.group_key].tracking_number}{if $shipments[$shipping.group_key].carrier_info.tracking_url}</a>{/if}){/if}</p>

                                        {$shipments[$shipping.group_key].carrier_info.info nofilter}
                                    {/if}
