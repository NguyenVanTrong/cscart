...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if ($mode == 'update') {
        ...
        if (!empty($_REQUEST['mailing_lists']) && !fn_is_empty($_REQUEST['mailing_lists'])) {
            ...
            $all_mailing_lists = array();
            if (!empty($_REQUEST['all_mailing_lists'])) {
                $all_mailing_lists = $_REQUEST['all_mailing_lists'];
            }

            if (!empty($subscriber)) {
                fn_delete_subscribers($subscriber['subscriber_id'], true, $all_mailing_lists);
            }
        }
    }
