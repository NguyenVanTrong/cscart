...
$meta = fn_ex_twg_init_api_meta($response);
...
if (($_SERVER['REQUEST_METHOD'] == 'POST' || $is_cache_request) && $mode == 'post') {
    ...
    if ($meta['action'] == 'login') {
        ...
        if (!$user_data = fn_ex_twg_api_customer_login($login, $password)) {
            $response->addError(
        ...
        $_profile = array_merge(
            $profile,
            array('cart' => fn_ex_twg_api_get_session_cart($_SESSION['cart'], $lang_code))
        );
        ...
        // add to cart
        $data = fn_ex_twg_get_api_data($response, $format);

        Registry::set('runtime.controller', 'checkout', true);
        $ids = fn_ex_twg_api_add_product_to_cart(array($data), $_SESSION['cart']);
        Registry::set('runtime.controller', 'twigmo');

        $result = fn_ex_twg_api_get_session_cart($_SESSION['cart'], $lang_code);
        ...
        $result = fn_ex_twg_api_get_session_cart($cart, $lang_code);
        $response->setData($result);
        ...
        $response->setData(fn_ex_twg_api_get_session_cart($cart));
        ...
        if (empty($order_id)) {
            if (!fn_ex_twg_set_internal_errors($response, 'ERROR_FAIL_POST_ORDER')) {
                $response->addError(
        ...
        if ($meta['object'] == 'cart') {
            ...
            if (!empty($data['products'])) {
                fn_ex_twg_api_add_product_to_cart($data['products'], $cart);
            }
            ...
            $profile = fn_ex_twg_get_user_info($_SESSION['auth']['user_id']);
            $profile = array_merge($profile, array('cart' => fn_ex_twg_api_get_session_cart($_SESSION['cart'], $lang_code)));
        ...
        if ($meta['object'] == 'page') {
            $response->setData(fn_ex_twg_api_get_page($_REQUEST['page_id']));
            ...
            fn_ex_twg_set_response_products(
                $response,
                $_REQUEST,
                $items_per_page,
                $lang_code
            ...
            if (fn_allowed_for('MULTIVENDOR')) {
                fn_ex_twg_add_response_vendors($response, $_REQUEST);
            }
        } elseif ($meta['object'] == 'categories') {
            fn_ex_twg_set_response_categories(
                $response,
                $_REQUEST,
                $items_per_page,
                $lang_code
            ...
            fn_ex_twg_set_response_catalog(
                $response,
                $_REQUEST,
                $items_per_page,
                $lang_code
            ...
            fn_ex_twg_set_response_homepage($response);
            ...
            if (!empty($payment_methods['payment'])) {
                foreach ($payment_methods['payment'] as $k => $payment_method) {
                    $options = fn_ex_twg_get_payment_options($payment_method);
                    if ($options) {
                ...
                $response->setData(
                    array(
                        ...
                        'cart' => fn_ex_twg_api_get_session_cart($cart, $lang_code)
                    )
                );
            }
            ...
            Registry::set('runtime.controller', 'checkout', true);
            $product_groups = fn_ex_twg_api_get_shippings($params);
            Registry::set('runtime.controller', 'twigmo');
            ...
            list($_reward_points_log, $params) = fn_ex_twg_get_reward_points_userlog(array(
        ...
        if ($meta['object'] == 'products') {
            ...
            Registry::set('runtime.checkout', false);
            $object = fn_ex_twg_get_api_product_data($_REQUEST['id'], $lang_code);
            $title = 'product';
            ...
            $object = fn_ex_twg_get_api_category_data($_REQUEST['id'], $lang_code);
            $title = 'category';
            ...
            $_auth = & $_SESSION['auth'];
            if (!empty($_auth['user_id'])) {
                $response->setData(fn_ex_twg_get_user_info($_auth['user_id']));
            } else {
    ...
    if (!empty($_REQUEST['close_notice']) && $_REQUEST['close_notice'] == 1) {
        ...
        $settings = fn_ex_twg_get_all_settings();
        echo 'settings=' . html_entity_decode(json_encode($settings), ENT_COMPAT, 'UTF-8');
        die();
    } elseif ($meta['action'] == 'send_order_push') {
        fn_ex_twg_show_fake_image();
        fn_ex_twg_send_notification();
        die();
    }
}

function fn_ex_twg_init_api_meta($response)
{
    // init request params
    $meta = array (
...
function fn_ex_twg_send_notification()
{
    if (!empty($_SESSION['twg_state']['send_push_for_order_ids'])) {
        $connector = new TwigmoConnector();
        fn_ex_twg_send_order_push_notification($_SESSION['twg_state']['send_push_for_order_ids'], $connector);
