...
use Tygh\Settings;
use ExTwigmo\Core\TwigmoSettings;
use ExTwigmo\Core\Api;
use ExTwigmo\Core\Functions\Lang;
use ExTwigmo\Core\Functions\BlockManager\TwigmoBlock;
use ExTwigmo\Core\Functions\Image\TwigmoImage;
use ExTwigmo\Core\TwigmoConnector;

require_once(Registry::get('config.dir.addons') . 'mobile_admin_app/ExTwigmo/Core/Functions/fn.common.php');
require_once(Registry::get('config.dir.addons') . 'mobile_admin_app/ExTwigmo/Core/TwigmoSettings.php');

function fn_ex_twg_get_lang_var($var_name, $lang_code = CART_LANGUAGE, $params = array())
{
    // Order params changed
    return __($var_name, $params, $lang_code);
}

function fn_ex_twg_get_device_type()
{
...
function fn_ex_twg_get_ajax_reconnect_code($mode = 'update_twigmo_connection')
{
...
function fn_ex_twg_get_view_object()
{
...
function fn_mobile_admin_app_get_shipments(&$params, &$fields_list, &$joins, &$condition, &$group)
{
...
function fn_mobile_admin_app_get_users(&$params, &$fields, &$sortings, &$condition, &$join)
{
    $twg_condition = fn_ex_twg_get_users_search_condition($params);
    if ($twg_condition) {
        $condition['twg_users'] = $twg_condition;
    }
}

function fn_mobile_admin_app_set_admin_notification()
{
    if (!TwigmoConnector::anyFrontendIsConnected()) {
        $message = __('twgadmin_connect_notification', array('[addon_link]' => fn_url('addons.update&addon=mobile_admin_app')));
        fn_set_notification('N', __('notice'), $message, 'S');
    }
    if (fn_ex_twg_is_on_saas()) {
        if (!isset($_SESSION['stats'])) {
            $_SESSION['stats'] = array();
        }
        $_SESSION['stats'][] = fn_ex_twg_get_ajax_reconnect_code('check_twigmo_for_upgrade');
    }
}


function fn_mobile_admin_app_get_categories(
    &$params,
    &$join,
    &$condition,
    &$fields,
 ...
 */
function fn_mobile_admin_app_order_placement_routines(
    &$order_id,
    &$force_notification,
    &$order_info,
    &$_error,
 ...
 */
function fn_mobile_admin_app_user_init(&$auth, &$user_info, &$first_init)
{
 ...
 */
function fn_mobile_admin_app_get_pages(
    &$params,
    &$join,
    &$condition,
    &$fields,
 ...
 */
function fn_mobile_admin_app_redirect_complete(&$meta_redirect = null)
{
 ...
 * @return null
 */
 function fn_mobile_admin_app_shippings_get_shippings_list($group, $shippings, &$condition)
 {
...
function fn_mobile_admin_app_place_order($order_id, $action = '', $__order_status = '', $cart = null)  // Hook
{
...
function fn_mobile_admin_app_order_notification($order_info, $order_statuses, $force_notification)  // Hook
{
    $connector = new TwigmoConnector();
    $company_id = fn_allowed_for('ULTIMATE') ? $order_info['company_id'] : 0;
    fn_ex_twg_send_order_status_push_notification($order_info, $order_statuses, $force_notification, $connector, $company_id);
}
 ...
 * @return boolean
 */
function fn_ex_twg_use_https_for_customer($company_id = 0)
{
    if (!$company_id) {
        $company_id = fn_ex_twg_get_current_company_id();
    }
 ...
 */
function fn_ex_twg_write_to_file($file_name, $data, $serialize = true)
{
...
function fn_ex_twg_save_version_info($version_info)
{
    fn_ex_twg_write_to_file(TWIGMO_UPGRADE_DIR . TWIGMO_UPGRADE_VERSION_FILE, $version_info);
}

function fn_ex_twg_get_carriers()
{
    return array (
        'USP'=> __('usps'),
        'UPS'=> __('ups'),
...
function fn_ex_twg_get_domain_name($host)
{
    $parts = explode('.', $host);
 ...
 */
function fn_ex_twg_api_get_products($params, $items_per_page = 10, $lang_code = CART_LANGUAGE)
{
...
function fn_ex_twg_api_get_categories($params, $lang_code = CART_LANGUAGE)
{
    $params['get_images'] = 'Y';
...
function fn_ex_twg_api_get_product_options($product, $lang_code = CART_LANGUAGE)
{
...
function fn_ex_twg_get_api_product_data($product_id, $lang_code = CART_LANGUAGE)
{
    $auth = & $_SESSION['auth'];
    ...
    $product_options = fn_ex_twg_api_get_product_options($product);
    ...
    $product['product_options_exceptions'] = fn_ex_twg_get_api_product_options_exceptions($product_id);
    $product['product_options_inventory'] = fn_ex_twg_get_api_product_options_inventory($product_id, $lang_code);
    ...
    $_product['TIME'] = TIME;
    if (AREA == 'C') {
        $_product['tabs'] = fn_ex_twg_get_product_tabs(array('product_id' => $product_id, 'descr_sl' => DESCR_SL));
    }
...
function fn_ex_twg_get_product_tabs($params)
{
...
function fn_ex_twg_get_api_category_data($category_id, $lang_code)
{
...
function fn_ex_twg_get_api_product_options_exceptions($product_id)
{
...
function fn_ex_twg_get_api_product_options_inventory($product_id, $lang_code = CART_LANGUAGE)
{
...
function fn_ex_twg_api_customer_login($user_login, $password)
{
...
function fn_ex_twg_api_customer_logout()
{
...
function fn_ex_twg_cmp_products($products1, $products2)
{
...
function fn_ex_twg_cmp_products2($product1, $product2)
{
...
function fn_ex_twg_filter_payment_buttons($payment_buttons)
{
    if (!$payment_buttons) {
...
function fn_ex_twg_api_get_cart_promotion_input_field()
{
    $input_field = 'N';
...
function fn_ex_twg_get_payment_methods()
{
...
function fn_ex_twg_api_get_session_cart(&$cart, $lang_code = CART_LANGUAGE)
{
    ...
    $payment_methods = fn_ex_twg_get_payment_methods();
    ...
    $cart_data['payment_buttons'] = fn_ex_twg_filter_payment_buttons($payment_buttons);
    // ================ /Payment buttons =======================================
...
function fn_ex_twg_api_get_cart_products($cart_items, $lang_code = CART_LANGUAGE)
{
    if (empty($cart_items)) {
        return array();
    }
...
function fn_ex_twg_api_add_product_to_cart($products, &$cart)
{
    $products_data = array();
...
function fn_ex_twg_get_random_ids($qty, $field, $table, $condition = '')
{
 ...
 */
function fn_ex_twg_get_category_product_ids($category_id, $get_sub  = false)
{
    ...
    if ($get_sub) {
        ...
        $categories = fn_ex_twg_api_get_categories($category_params);
 ...
 * @param ExTwigmo\ApiData $response
 * @param array $params array('items_per_page' => subj, 'page' => requested_page)
 * @param boolean $set_empty
 * @return boolean always true
 */
function fn_ex_twg_set_response_pagination(&$response, $params, $set_empty = false)
{
    $params = array(
...
function fn_ex_twg_set_response_products(&$response, $params, $items_per_page = 0, $lang_code = CART_LANGUAGE) {
    list($products, $request) = fn_ex_twg_api_get_products($params, $items_per_page, $lang_code);
    if (!empty($products)) {
    ...
    fn_ex_twg_set_response_pagination($response, $pagination_params);
}

function fn_ex_twg_set_response_categories(
    &$response,
    $params,
    $items_per_page = 0,
    ...
    if (empty($items_per_page)) {
        $result = fn_ex_twg_api_get_categories($params, $lang_code);
        $response->setMeta(
        ...
        $categories = fn_ex_twg_api_get_categories($params, $lang_code);
...
function fn_ex_twg_set_response_catalog(
    &$response,
    $params,
    $items_per_page = 0,
    $lang_code = CART_LANGUAGE
    ...
    if (empty($params['page']) || $params['page'] == 1) {
        ...
        $categories = fn_ex_twg_api_get_categories($category_params, $lang_code);
    ...
    if (!empty($params['category_id'])) {
        ...
        list($products, $params) = fn_ex_twg_api_get_products($params, $items_per_page, $lang_code);
    ...
    fn_ex_twg_set_response_pagination($response, $pagination_params, true);
}

function fn_ex_twg_api_get_base_statuses($add_hidden = true)
{
    $statuses = array (
        'A' => array(
            'status' => 'A',
...
function fn_ex_twg_api_get_object($response, $object_type, $params)
{
...
function fn_ex_twg_check_api_user_data(
    $user,
    $location = 'C',
    $lang_code = CART_LANGUAGE
) {
...
function fn_ex_twg_set_internal_errors(&$response, $error_code)
{
...
function fn_ex_twg_api_set_cart_user_data($user_data, $response, $lang_code = CART_LANGUAGE) {
    $cart = & $_SESSION['cart'];

    // User update or registration
    $_user = fn_ex_twg_parse_api_object($user_data, 'users');
    $user = fn_ex_twg_check_api_user_data($_user, 'C', $lang_code);
    if (empty($user)) {
        if (!fn_ex_twg_set_internal_errors($response, 'ERROR_FAIL_UPDATE_USER')) {
...
function fn_ex_twg_api_get_shippings(&$params)
{
 ...
 * @param string $lang_code
 */
function fn_ex_twg_add_response_vendors(&$response, $params)
{
    if (empty($params['q'])) {
        return;
    }
 ...
 */
function fn_ex_twg_set_response_homepage(&$response)
{
    ...
    if (
        ...
        if ($home_page_content == 'random_products') {
            $product_ids = fn_ex_twg_get_random_ids(
            ...
            $product_ids =
                fn_ex_twg_get_category_product_ids($home_page_content, false) or array();
            $block['title'] = fn_get_category_name($home_page_content);
        }
        list($block['products']) = fn_ex_twg_api_get_products(
 ...
 * @return array
 */
function fn_ex_twg_get_locations_info()
{
    ...
    $defaultLayoutId = fn_ex_twg_get_default_layout_id();
 ...
 * @return integer
 */
function fn_ex_twg_get_default_layout_id()
{
    $condition = "";
    if (fn_allowed_for('ULTIMATE')) {
 ...
 */
function fn_ex_twg_prepare_profile_fields($fields, $only_reguired)
{
 ...
 */
function fn_ex_twg_get_banner_url($banner_id, $url)
{
...
function fn_ex_twg_get_twigmo_onclick($url)
{
    $onclick = array();
    // Process SEO links
...
function fn_ex_twg_get_page_onclick($url, $page_type, $page_id)
{
    $onclick = array(
        'actionType' => 'cmsPage',
        'actionId'   => $page_id
    ...
    if ($page_type == 'L' && $url) {
        $url_onclick = fn_ex_twg_get_twigmo_onclick($url);
 ...
 * @param int $banner_id
 */
function fn_ex_twg_get_banner_onclick($url, $target, $type, $banner_id)
{
    $onclick = array();
    if ($target == 'T' and !empty($url) and $type == 'G') {
        $url = fn_ex_twg_get_banner_url($banner_id, $url);
        $onclick = fn_ex_twg_get_twigmo_onclick($url);
    }
 ...
 * Get user info
 */
function fn_ex_twg_get_user_info($user_id)
{
    $profile = array();
    if (!$user_id) {
        $profile['user_id'] = 0;
 ...
 * @param int $page_id
 */
function fn_ex_twg_api_get_page($page_id)
{
    if (!$page_id) {
        return false;
    }
 ...
 */
function fn_ex_twg_api_get_form_elements($elements)
{
    $result = array();
    if ($elements) {
 ...
 * @param array $page_id
 */
function fn_ex_twg_api_get_form_info($form)
{
    if (!$form) {
        return false;
    }
    ...
    return array(
        ...
        'elements' => fn_ex_twg_api_get_form_elements($form['elements'])
    );
}
 ...
 * @param int $grid_id
 */
function fn_ex_twg_is_twigmo_grid($grid_id)
{
    ...
    return fn_ex_twg_is_twigmo_location($container['location_id']);
}
 ...
 * @param int $grid_id
 */
function fn_ex_twg_is_twigmo_location($location_id)
{
 ...
 * @return string
 */
function fn_ex_twg_get_external_info_url($url)
{
    $url = trim($url);
    if (!$url) {
        return '';
 ...
 */
function fn_ex_twg_get_sortings()
{
...
function fn_ex_twg_get_feature_value($value, $feature_type, $value_int, $variant_id, $variants)
{
    if ($feature_type == "D") {
...
function fn_ex_twg_get_cache_request($request)
{
 ...
 */
function fn_ex_twg_get_boot_settings()
{
    $settings = array();
    ...
    $settings['cacheRequest'] = fn_ex_twg_get_cache_request($settings['request']);

    $controller = $addon_settings['home_page_content'] == 'home_page_blocks' ? 'index.index' : 'twigmo.post';
    $settings['home_page_title'] = fn_ex_twg_get_location_page_title($controller);
...
function fn_ex_twg_get_api_data($response, $format, $required = true)
{
    $data = array();
...
function fn_ex_twg_filter_profile_fields($value)
{
    return $value !== NULL;
}

function fn_ex_twg_get_checkout_settings()
{
 ...
 */
function fn_ex_twg_get_all_settings()
{
    $settings = fn_ex_twg_get_boot_settings();
    $addon_settings = TwigmoSettings::get();
    ...
    $settings['url_for_facebook'] =
        isset($addon_settings['url_for_facebook']) ?
            fn_ex_twg_get_external_info_url($addon_settings['url_for_facebook']) :
            '';
    $settings['url_for_twitter'] =
        isset($addon_settings['url_for_twitter']) ?
            fn_ex_twg_get_external_info_url($addon_settings['url_for_twitter']) :
            '';

    $settings['lang'] = array_merge(fn_ex_twg_get_default_customer_langvars(), Lang::getCustomerLangVars());

    // Countries/states
    list($countries) = fn_get_countries(array('only_avail' => true));
    $settings = array_merge($settings, Api::getAsList('countries', $countries));
    $settings['states'] = fn_ex_twg_get_states();
    ...
    $settings['profileFields'] = fn_ex_twg_prepare_profile_fields(fn_get_profile_fields(), $only_required);
    $settings['profileFieldsCheckout'] = fn_ex_twg_prepare_profile_fields(fn_get_profile_fields('O'), $only_required);
    ...
    $settings['profile'] = fn_ex_twg_get_user_info($_SESSION['auth']['user_id']);

    $settings['cart'] = fn_ex_twg_api_get_session_cart($_SESSION['cart']);

    $settings['sortings'] = fn_ex_twg_get_sortings();
    ...
    $settings = array_merge($settings, fn_ex_twg_get_checkout_settings());
    ...
    $settings['lang'] = fn_ex_twg_process_langvars($settings['lang']);
 ...
 */
function fn_ex_twg_get_location_page_title($controller = 'twigmo.post')
{
...
// ====== Func for schemas =================
function fn_ex_twg_get_api_image_data($params)
{
...
function fn_ex_twg_process_ua($ua)
{
    $result = 'unknown';
    ...
    $ua_meta = fn_ex_twg_get_ua_meta($ua, $rules);
    // Save stat
    foreach ($ua_meta as $section => $value) {
        $where = array('section' => $section, 'value' => $value, 'month' => date('Y-m-1'));
        $count = db_get_field('SELECT count FROM ?:mobile_admin_app_ua_stat WHERE ?w', $where);
        if ($count) {
            db_query('UPDATE ?:mobile_admin_app_ua_stat SET count=count+1 WHERE ?w', $where);
        } else {
            $where['count'] = 1;
            db_query('INSERT INTO ?:mobile_admin_app_ua_stat ?e', $where);
        }
    }
...
function fn_ex_twg_get_ua_meta($ua, $ruleSections)
{
    $results = array();
    $ua = strtolower($ua);
    foreach ($ruleSections as $section => $rules) {
        $results[$section] = fn_ex_twg_check_ua_rule($rules['rules'], $ua, $results);
    }
...
function fn_ex_twg_check_ua_rule($rules, $ua, $results)
{
    $result = '';
    foreach ($rules as $rule) {
        ...
        if (preg_match($rule['expression'], $checked_value) xor isset($rule['is_filter'])) {
            ...
            if (isset($rule['rules'])) {
                $subrelsResult = fn_ex_twg_check_ua_rule($rule['rules'], $ua, $results);
                if ($subrelsResult) {
 ...
 * @return string
 */
function fn_ex_twg_add_path_to_url($url)
{
    return Registry::get('config.current_path') . '/' . $url;
}

function fn_ex_twg_get_saas_uid()
{
    return intval(Registry::get('config.saas_uid'));
}

function fn_ex_twg_is_on_saas()
{
    $saas_uid = fn_ex_twg_get_saas_uid();
 ...
 * @return integer
 */
function fn_ex_twg_get_current_company_id()
{
    if (!fn_allowed_for('ULTIMATE')) {
        return 0;
    }
...
function fn_ex_twg_get_images_path()
{
    return TwigmoImage::getImagesPath();
}

/**
 * Check if stores have to be reconnected and reconnects them after store mode changed
 *
 * @return bool True if reconnect happened
 */
function fn_ex_twg_reconnect_on_license_change()
{
    if (isset(Tygh::$app['session']['twg_prev_store_mode'])) {
        $current_store_mode = fn_get_storage_data('store_mode');
        if ($current_store_mode != 'trial' &&
            $current_store_mode != Tygh::$app['session']['twg_prev_store_mode']
        ) {
            if (fn_allowed_for('ULTIMATE:')) {
                fn_update_addon_status('mobile_admin_app', 'D', true, false, false);
            } else {
                // reconnect stores
                $connector = new TwigmoConnector();
                if ($connector->updateConnections(true)) {
                    $connector->displayServiceNotifications();
                    unset(Tygh::$app['session']['twg_prev_store_mode']);
