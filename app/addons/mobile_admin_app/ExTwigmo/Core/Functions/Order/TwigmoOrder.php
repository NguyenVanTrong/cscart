...
namespace ExTwigmo\Core\Functions\Order;
...
class TwigmoOrder
{
    ...
    public static function apiPlaceOrder($data, &$response, $lang_code = CART_LANGUAGE)
    {
        ...
        if (!empty($data['user'])) {
            fn_ex_twg_api_set_cart_user_data(
                $data['user'],
                $response,
                $lang_code
            );
        ...
        $cart['details'] = fn_ex_twg_get_twigmo_order_note();
        Registry::set('runtime.controller', 'checkout', true);
    ...
    public static function getOrderInfo($order_id)
    {
        ...
        $object['date'] = fn_ex_twg_format_time($object['timestamp']);
    ...
    public static function returnPlacedOrders($order_id, &$response, $items_per_page, $lang_code) {
        ...
        if (!in_array($status, fn_get_order_paid_statuses())) {
            ...
            if ($status != 'B') {
                ...
                if (!fn_ex_twg_set_internal_errors($response, 'ERROR_ORDER_BACKORDERED')) {
                    $response->addError(
