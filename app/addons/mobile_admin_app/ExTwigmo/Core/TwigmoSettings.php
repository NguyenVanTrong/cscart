...
{
    const REGISTRY_PATH = 'runtime.mobile_admin_app';

    /**
     * @param string $path Path to settings in the 'runtime.mobile_admin_app'
     * @return mixed
     */
    ...
    {
        $settings = db_get_hash_single_array('SELECT * FROM ?:mobile_admin_app_settings', array('name', 'value'));
        $store_sql = 'SELECT * FROM ?:mobile_admin_app_stores WHERE type = ?s';
    ...
    {
        if (empty($store)) {
            db_query('DELETE FROM ?:mobile_admin_app_stores WHERE type = ?s', $type);
            return;
        }
        ...
        $is_old_store = db_get_field("SELECT count(*) FROM ?:mobile_admin_app_stores $condition", $store['company_id'], $type);
        $store = array_merge($store, self::_saveUploadedLogos());
        $store['type'] = $type;
        if ($is_old_store) {
            db_query("UPDATE ?:mobile_admin_app_stores SET ?u $condition", $store);
        } else {
            $store['repo_revision'] = TIME;
            db_query('INSERT INTO ?:mobile_admin_app_stores ?e', $store);
        }
    }
    ...
    private static function _setCommonSettings($settings)
    {
        ...
        foreach ($settings as $name => $value) {
            db_query('REPLACE INTO ?:mobile_admin_app_settings ?e', array('name' => $name, 'value' => $value));
        }
    }
