...
require_once(Registry::get('config.dir.addons') . 'mobile_admin_app/ExTwigmo/Core/AbstractTwigmoConnector.php');
...
class TwigmoConnector extends \AbstractTwigmoConnector
{
    ...
    const ENGINE_NAME =         'cscart';
    const ADDON_NAME =          'mobile_admin_app';
    ...
    public function authPage($page)
    {
        ...
        if (!$this->responseIsOk($response)) {
            die(str_replace(
                ...
                array('<a href="' . fn_url('addons.update&addon=mobile_admin_app') . '">', '</a>'),
    ...
    public function connectAdmin($user_data, $is_reconnect = false)
    {
        ...
        $data = array(
            ...
            'engine_license_number' => Settings::instance()->getValue('license_number', 'Upgrade_center'),
        );
    ...
    public function connect($stores = array(), $user_data = array(), $is_reconnect = false, $is_force_reconnect = false)
    {
        ...
        // Connect frontends
        $all_stores = fn_ex_twg_get_stores();
    ...
    public static function getCustomerUrl($store, $protocol = 'current') // protocol may be current, http, https
    {
        ...
        if ($protocol == 'current') {
            $protocol = fn_ex_twg_use_https_for_customer($store['company_id']) ? 'https' : 'http';
        }
    ...
    public static function updateUARules($access_id = '')
    {
        ...
        fn_ex_twg_write_to_file(TWIGMO_UA_RULES_FILE, $rules_on_twigmo, false);
    }
