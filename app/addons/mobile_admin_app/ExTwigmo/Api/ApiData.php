...
namespace ExTwigmo\Api;

use Tygh\Registry;
use ExTwigmo\Api\TwgApiv2;
use ExTwigmo\Api\TwgApi;
...
class ApiData
{
    ...
    public function __construct(
        ...
        if (fn_allowed_for('ULTIMATE:')) {
            $this->addError(str_rot13('REEBE_NPPRFF_QRAVRQ'), __(str_rot13('npprff_qravrq')));
            $this->returnResponse();
        }
    }
    ...
    {
        if ($format == 'xml') {
            $result = @simplexml_load_string(SecurityHelper::removeXmlEntities($data), 'SimpleXMLElement', LIBXML_NOCDATA);
