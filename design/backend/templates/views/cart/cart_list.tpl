...
<tr>
    ...
    <td>
        ...
        {if $customer.user_data.email}<a href="{"profiles.update?user_id=`$customer.user_id`"|fn_url}" class="underlined">{if $customer.firstname || $customer.lastname}{$customer.lastname} {$customer.firstname}{else}{$customer.user_data.email}{/if}</a>{else}{__("unregistered_customer")} {if $customer.lastname || $customer.firstname}<p>{$customer.lastname} {$customer.firstname} {if $customer.email}({$customer.email}){/if}{/if}</p>{/if}
