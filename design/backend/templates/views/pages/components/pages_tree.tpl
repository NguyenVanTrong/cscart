...
<tr class="cm-row-status-{$page.status|lower}" {if $page.level > 0 && $search.get_tree == "multi_level"}class="multiple-table-row"{/if}>
    ...
    <td class="row-status" width="70%">
        ...
        <div class="text-over" {if $search.get_tree == "multi_level"}style="padding-{$direction}: {math equation="x*14" x=$page.level|default:0}px;"{/if}>
            ...
            {/if}
            <span alt="{__("expand_sublist_of_items")}" title="{__("expand_sublist_of_items")}" id="on_page{$combination_suffix}_{$page.page_id}" class="cm-combination-pages{$combination_suffix} {if $expand_all && !$hide_show_all}hidden{/if}" {if $page.has_children}onclick="Tygh.$.ceAjax('request', '{"$_dispatch?parent_id=`$page.page_id`&get_tree=multi_level`$except_url`&display=`$display`&checkbox_name=`$checkbox_name`&combination_suffix=`$combination_suffix`{if $is_exclusive_page_type}&page_type={$page.page_type}{/if}"|fn_url nofilter}', {$ldelim}result_ids: 'page{$combination_suffix}_{$page.page_id}', caching: true{$rdelim});"{/if}><span class="icon-caret-right"></span></span>
            <span alt="{__("collapse_sublist_of_items")}" title="{__("collapse_sublist_of_items")}" id="off_page{$combination_suffix}_{$page.page_id}" class="cm-combination-pages{$combination_suffix} {if !$expand_all || $hide_show_all}hidden{/if}"><span class="icon-caret-down"></span> </span>
