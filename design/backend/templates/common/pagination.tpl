        ...
        {if $pagination.total_items > $min_per_page_range}
            <div class="pagination pull-left">
                ...
                {if $pagination.current_page != "full_list" && $pagination.total_pages > 0}
                    <li class="{if !$pagination.prev_page}disabled{/if}{$history_class}"><a data-ca-scroll=".cm-pagination-container" class="cm-ajax{$history_class}" {if $pagination.prev_page}href="{"`$c_url`&page=`$pagination.prev_page`"|fn_url}" data-ca-page="{$pagination.prev_page}" data-ca-target-id="{$id}"{/if}> <i class="icon icon-angle-left"></i>&nbsp;{__("previous")}</a></li>
                    ...
                    {/foreach}
                    <li class="{if !$pagination.next_page}disabled{/if}{$history_class}"><a data-ca-scroll=".cm-pagination-container" class="{if $pagination.next_page}cm-ajax{/if}{$history_class}" {if $pagination.next_page}href="{"`$c_url`&page=`$pagination.next_page`"|fn_url}" data-ca-page="{$pagination.next_page}" data-ca-target-id="{$id}"{/if}>{__("next")}&nbsp;<i class="icon icon-angle-right"></i></a></li>
                {/if}
                </ul>
            </div>
