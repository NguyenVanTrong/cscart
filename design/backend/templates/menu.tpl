...
<div class="navbar-admin-top">
    ...
    <div class="navbar navbar-inverse" id="header_navbar">
        ...
        {/if}
        {*
        {if $smarty.const.PRODUCT_STATUS}
            {$tracker_url = parse_url($config.resources.bug_tracker_url)}
            {parse_str($tracker_url.query, $tracker_url.query)}
            <ul class="nav">
                <li class="divider-vertical"></li>
                <li>
                    <a class="cm-submit cm-new-window bug-report" data-ca-target-form="elm_report_a_bug" >
                        <i class="icon-white icon-bug"></i>
                        {__("report_a_bug")}
                    </a>
                    <form method="POST" action="{$tracker_url.scheme}://{$tracker_url.host}{$tracker_url.path}" style="display: none;" name="elm_report_a_bug">
                        {foreach $tracker_url.query as $param_name => $param_value}
